    ...
    var MediaPlayer = (function () {
        function MediaPlayer(element, options) {
            ...
            for (var _b = 0, _c = Array.from(this.head.querySelectorAll('source')); _b < _c.length; _b++) {
                var el = _c[_b];
                if ((el.type == 'application/dash+xml' || el.type == 'application/x-mpegURL') && !this.head.canPlayType(el.type)) {
                    var controller = new shaka.Player(this.head);
                    var bandwidth = 2500000;
                    ...
                    controller.addEventListener('error', function (err) {
                        console.error('load error', err);
                    });
                    controller.load(el.src).then(function (r) {
                        if (_this.playRequested) {
                            _this.play();
                        }
                    });
                    this.head.shakaPlayer = controller;
                    this.shakaPlayer = controller;
                }
        ...
        Object.defineProperty(MediaPlayer.prototype, "isFullscreen", {
            get: function () {
